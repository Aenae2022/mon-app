...
export default function ListWords({ listWords, cibleSrc, indexList }) {
        ...
        return (
          <>
            <img
              ...
              src={
                status === "nope"
                  ? "/asset/picture/icons/faux.png"
                  : "/asset/picture/icons/vrai.png"
              }
              alt="résultat"
            />
            <br />
  ...
  function handleClickApprendre(w) {
    ...
    const redirectionWord =
      "/dictation/word/" + indexList + "/" + wordIndex + "/1"
      ...
      <>
        <ul>
          {wordsDatas.map((w, i) => {
            ...
            return (
              <li key={w.word} className={`wordInList ${myClassName}`}>
                {w.word} {myContent}
              </li>
            )
          })}
